...
function imgDir(root, curCatInd, curImgInd) {
    return root + IMG_DIR_CAT[curCatInd] + curImgInd + IMG_EXT;
}

async function loadImages() {
    for (let i = 0; i < IMG_SCROLL_COUNT; ++i) {
        isLoadingImages = true;
        if (curScrollImgInd >= IMG_COUNT[currentCatInd]) {
            break;
        }
        let img = document.createElement("img");
        img.className = IMG_CSS_CLASS;
        img.src = imgDir(IMG_DIR, currentCatInd, curScrollImgInd);
        img.alt = IMG_ALT;
        imgColumns[curColInd].appendChild(img);
        let zoomSrc = imgDir(IMG_ZOOM_DIR, currentCatInd, curScrollImgInd);
        let zoomImgIdx = curScrollImgInd;
        img.addEventListener("click", () => {
            enableImgZoom(zoomSrc, zoomImgIdx);
        });
...
let curZoomImgIndex = 0;
let resettedZoom = false;
let imgZoomEnabled = false;
let curZoomPercentIndex = 0;
...
imgZoomPrevBtn.addEventListener("click", () => {
    loadAdjacentZoomImg(false);
});
const imgZoomNextBtn = document.getElementById("img-zoom-next-btn");
imgZoomNextBtn.addEventListener("click", () => {
    loadAdjacentZoomImg(true);
});
...
function loadAdjacentZoomImg(isNext){
    if (isNext) {
        if (curZoomImgIndex + 1 < IMG_COUNT[currentCatInd]) {
            ++curZoomImgIndex;
            imgZoomSrc.src = imgDir(IMG_ZOOM_DIR, currentCatInd, curZoomImgIndex);
            if (curZoomImgIndex > curScrollImgInd) {
                loadImages();
            }
            zoomStyleReset();
        }
    } else {
        if (curZoomImgIndex - 1 >= 0) {
            --curZoomImgIndex;
            imgZoomSrc.src = imgDir(IMG_ZOOM_DIR, currentCatInd, curZoomImgIndex);
            zoomStyleReset();
        }
...
function isMinWidth(){
    return document.body.offsetWidth <= 550;
}

function enableImgZoom(source, curImgIndex){
    imgZoomEnabled = true;
    imgZoomWrapper.style.display = "block";
    imgZoomSrc.src = source;
    curZoomImgIndex = curImgIndex;
    curZoomPercentIndex = 0;
    zoomStyleReset();
}

function zoomStyleReset() {
    if (!isMobile && !isMinWidth()) {
        imgZoomSrc.style.height = zoomPercents[curZoomPercentIndex];
...
function imgZoom(){
    ...
    resettedZoom = false;
    ++curZoomPercentIndex;
    if(curZoomPercentIndex >= zoomPercents.length) {
        curZoomPercentIndex = 0;
    }
    imgZoomSrc.style.height = zoomPercents[curZoomPercentIndex];
}
